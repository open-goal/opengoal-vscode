...
(function (root, factory) {
  ...
  function getInitialResult(text, options, mode, smart) {
    var result = {
      ...
      inputLines: text.split(LINE_ENDING_REGEX),
  ...
  var errorMessages = {};
  errorMessages[ERROR_QUOTE_DANGER] =
    "Quotes must balanced inside comment blocks.";
  errorMessages[ERROR_EOL_BACKSLASH] =
    "Line cannot end in a hanging backslash.";
  ...
  errorMessages[ERROR_LEADING_CLOSE_PAREN] =
  ...
  function isCursorAffected(result, start, end) {
    if (result.cursorX === start && result.cursorX === end) {
  ...
  function checkCursorHolding(result) {
    ...
    var holding =
      ...
      holdMinX <= result.cursorX &&
      result.cursorX <= holdMaxX;
    var shouldCheckPrev =
      !result.changes && result.prevCursorLine !== UINT_NULL;
    if (shouldCheckPrev) {
      var prevHolding =
        result.prevCursorLine === opener.lineNo &&
        holdMinX <= result.prevCursorX &&
        result.prevCursorX <= holdMaxX;
  ...
  function onUnmatchedCloseParen(result) {
    if (result.mode === PAREN_MODE) {
      ...
      var inLeadingParenTrail =
        trail.lineNo === result.lineNo && trail.startX === result.indentX;
      ...
      result.mode === INDENT_MODE &&
      !result.errorPosCache[ERROR_UNMATCHED_CLOSE_PAREN]
    ) {
  ...
  function cleanParenTrail(result) {
    ...
    if (startX === endX || result.lineNo !== result.parenTrail.lineNo) {
      return;
    }
  ...
  function updateRememberedParenTrail(result) {
    ...
    if (!trail || trail.lineNo !== result.parenTrail.lineNo) {
      ...
      if (result.returnParens) {
        var opener =
          result.parenTrail.openers[result.parenTrail.openers.length - 1];
  ...
  function checkLeadingCloseParen(result) {
    if (
      result.errorPosCache[ERROR_LEADING_CLOSE_PAREN] &&
  ...
  function getTabStopLine(result) {
    return result.selectionStartLine !== UINT_NULL
      ? result.selectionStartLine
      : result.cursorLine;
  }
